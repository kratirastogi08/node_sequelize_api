...
const placeOrder=async(req,res,next)=>{
    ...
    try{
       ...
       orderItems.forEach(i=>{
        ...
        const order=await Orders.create({...rest,userId,trackingNumber,totalPrice},{transaction:t})
        for(let item of orderItems) {
            await OrderItems.create({...item,userId:1,orderId:order.dataValues.id,},{transaction:t})
               ...
               await Products.increment({quantity: -item.quantity}, { where: { id: item.productId } },{transaction:t})
...
const cancelOrder=async(req,res,next)=>{
    ...
    if(order.dataValues.orderStatus==='Delivered'||order.dataValues.orderStatus==='Shipped')
     {
        return response.error(res,400,"Since the order is already shipped or delivered and hence order cannot be cancelled")  
     }
